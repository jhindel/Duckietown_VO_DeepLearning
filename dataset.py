...
class DuckietownDataset(Dataset):
    ...
    def __init__(self, data_dic, args):
        ...
        for i in range(len(data_dic["filenames"])):
            # read .txt file
            gt_file = pd.read_fwf(data_dic["sub_folder"] + data_dic["filenames"][i])
            # print(i, gt_file.shape)
            # get list of all files in dir
            all_filenames_dir = sorted(next(walk(data_dic["sub_folder"] + data_dic["dir"][i]), (None, None, []))[2])
            full_path = np.array(
                [os.path.join(str(data_dic["sub_folder"] + data_dic["dir"][i]), xi) for xi in all_filenames_dir])
        ...
        print("batches", (len(self.data) - 1) // self.trajectory_length)
...
if __name__ == "__main__":
    ...
    args = {"datapath": 'drive/My Drive/VOdata/alex_3small_loops', "checkpoint_path": './Checkpoint',
            "checkpoint": None,
            "bsize": 1, "lr": 0.001, "weight_decay": 1e-4, "trajectory_length": 5, "dropout_p": 0.85,
            "resize": 64, "K": 100, "epochs": 100, "patience": 40, "camera-correction": True}

    test_dataset = DuckietownDataset(train_dummy, args)
